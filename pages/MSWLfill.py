...
def create_realistic_cross_section_profile(points, cross_section_pos, is_vertical, results_data, bund_h, external_slope_param, at_depth, anchor_depth, anchor_width, waste_h, berm_w):
...
with tab3:
    ...
    with col1:
        ...
        if cross_section_type == "Vertical (Y-axis)":
            ...
            line_z = np.linspace(-depth_below_ngl - anchor_trench_depth, results['max_height'], 20)
        ...
        fig_3d.update_layout(
            title="3D Landfill Model with Proper Structure",
            scene=dict(
    ...
    with col2:
        ...
        # Create 2D cross-section with proper landfill structure
        fig_2d = go.Figure()
        ...
        try:
            ...
            if is_vert:
                ...
                waste_profile, bund_profile = create_realistic_cross_section_profile(
                    ...
                    at_depth=depth_below_ngl,
                    anchor_depth=anchor_trench_depth,
                    anchor_width=anchor_trench_width,
                    waste_h=waste_height,
                    berm_w=berm_width
                )
                ...
                # Add anchor trench
                fig_2d.add_trace(go.Scatter(
                    x=y_points, y=[-depth_below_ngl - anchor_trench_depth]*len(y_points),
                    mode='lines',
                    line=dict(color='brown', width=2, dash='dash'),
                    name='Anchor Trench Bottom',
                    hoverinfo='skip'
                ))
                ...
                for level in results['levels']:
                    if level['Volume'] > 0:
                        if level['Type'] in ['Excavation', 'Waste up to Bund', 'Waste above Bund']:
                            l, w = level['Length'], level['Width']
                            if abs(cs_x) <= l/2:
                                fig_2d.add_hline(
                                    y=level['Top_Z'], 
                ...
                fig_2d.update_layout(
                    title="Vertical Cross-Section with Proper Structure",
                ...
                # Add anchor trench
                fig_2d.add_trace(go.Scatter(
                    x=x_points, y=[-depth_below_ngl - anchor_trench_depth]*len(x_points),
                    mode='lines',
                    line=dict(color='brown', width=2, dash='dash'),
                    name='Anchor Trench Bottom',
                    hoverinfo='skip'
                ...
                fig_2d.update_layout(
                    title="Horizontal Cross-Section with Proper Structure",
...
with tab4:
    ...
    with col2:
        ...
        anchor_trench_volume = sum(l['Volume'] for l in results['levels'] if l['Type'] == 'Anchor Trench')
        ...
        volume_breakdown = {
            'Component': ['Anchor Trench', 'Excavation Below Ground', 'Waste up to Bund', 'Waste above Bund', 'Total'],
            'Volume (Cum)': [anchor_trench_volume, excavation_volume, waste_up_to_bund, waste_above_bund, results['total_volume']],
            'Percentage': [
                f"{anchor_trench_volume/results['total_volume']*100:.1f}%" if results['total_volume'] > 0 else "0%",
